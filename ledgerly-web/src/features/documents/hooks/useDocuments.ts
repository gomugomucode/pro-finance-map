...
const INITIAL_DEMO_DOCUMENTS: DocumentItem[] = [
  {
    ...
    extracted_total: 1199.0,
    extracted_tax: 105.51,
    extracted_category: "Electronics & Hardware",
    extracted_raw_text:
      "Apple Store #R102. 1x iPhone 16 Pro Max 256GB Space Black $1199.00. Subtotal: $1199.00. Tax: $105.51. Total: $1304.51. VISA ending in 4242.",
  ...
  },
  {
    ...
    extracted_total: 299.0,
    extracted_tax: 24.0,
    extracted_category: "Warranty & Insurance",
    extracted_raw_text:
      "AppleCare+ Extended Warranty Certificate #AC-8849201. Serial: C02XG109P35. Coverage end: 2029-05-10.",
    ocr_provider: "ledgerly_local_ocr",
  ...
  },
  {
    ...
    extracted_total: 84500.0,
    extracted_tax: 14200.0,
    extracted_category: "Tax Documents",
    extracted_raw_text:
      "Form W-2 Wage and Tax Statement 2025. Employer: Acme Corp Inc. Federal Income Tax Withheld: $14,200.00.",
    ocr_provider: "ledgerly_local_ocr",
  ...
  },
  {
    ...
    extracted_total: 184.2,
    extracted_tax: 14.73,
    extracted_category: "Business Infrastructure",
    extracted_raw_text:
      "Amazon Web Services Invoice #99837162. Statement date: Jul 1, 2026. Total Amount Due: $184.20.",
    ocr_provider: "ledgerly_local_ocr",
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<DocumentItem[]> => {
      try {
        ...
        if (error) {
          console.warn(
            "Supabase documents query error, falling back to local vault storage:",
            error,
          );
          return applyFilters(INITIAL_DEMO_DOCUMENTS, filters);
        }
...
export function useCreateDocument() {
  ...
  return useMutation({
    mutationFn: async (payload: Partial<DocumentItem> & { file?: File }) => {
      ...
      let storagePath =
        payload.storage_path || `documents/${userId}/${Date.now()}_${payload.filename}`;
      ...
      let ocrData = {};
      if (payload.file) {
        ocrData = await defaultOcrEngine.extractDocumentData(
          payload.file,
