...
export async function runSmsEngineSelfTest() {
  ...
  if (chaseResult.amount_minor !== 4599)
    throw new Error("SMS Test 1 Failed: Amount minor mismatch");
  if (chaseResult.merchant !== "Starbucks Coffee")
    throw new Error("SMS Test 1 Failed: Merchant mismatch");
  ...
  if (amexResult.amount_minor !== 18950)
    throw new Error("SMS Test 2 Failed: Amex amount minor mismatch");
  if (amexResult.merchant !== "Target Stores")
    throw new Error("SMS Test 2 Failed: Amex merchant mismatch");

  // Test 3: Duplicate Message Detection
  const duplicateResult = engine.parseSms(chaseSms);
  if (!duplicateResult.is_duplicate)
    throw new Error("SMS Test 3 Failed: Duplicate detection failed");
  ...
  if (malformedResult.amount !== null)
    throw new Error("SMS Test 4 Failed: OTP message falsely extracted amount");
