...
import {
  ...
  CalendarEventItem,
} from "@/features/calendar/FinancialCalendarView";

const txQuery = queryOptions({
  queryKey: ["transactions"],
  queryFn: () => listTransactions({ data: { limit: 500 } }),
});
const subQuery = queryOptions({ queryKey: ["subscriptions"], queryFn: () => listSubscriptions() });
const recQuery = queryOptions({
...
export const Route = createFileRoute("/_authenticated/calendar")({
  ...
  pendingComponent: () => (
    <div className="p-8 text-sm text-muted-foreground">Loading calendar...</div>
  ),
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-10 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Visual aggregator of transactions, upcoming subscriptions, recurring rules, debt due
